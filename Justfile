...
build-frontend:
    ...
    cd src/frontend cd frontend && bun run build.tscd frontend && bun run build.ts bun run build.ts
...
format-backend:
    ...
    @ruff check --fix --select I src/backend

# Format TypeScript/React code
format-frontend:
    @echo "🎨 Formatting TypeScript..."
    @bun run prettier --write src/frontend/**/*.{ts,tsx,json,css}
    @bun run eslint src/frontend/**/*.{ts,tsx} --fix
...
lint-frontend:
    ...
    @bun run eslint src/frontend/**/*.{ts,tsx}

# Run security checks on backend
security-backend:
    @echo "🔒 Running security checks..."
    @bandit -r src/backend -ll
    @gitleaks detect --source src/backend --no-git --no-banner
